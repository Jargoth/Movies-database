...
{
	if (($_COOKIE['username'] AND $_COOKIE['password']) AND !$_GET['all'])
	{
    ...
    $SQL = "SELECT a.owner, u.\"user\" FROM movies.accessrights a, web.users u WHERE u.id = a.owner AND a.permission = TRUE AND a.\"view\" = TRUE AND a.\"user\" = ".$user[0]['id'];
	  ...
	  $SQL = "SELECT m.title, m.id, m.year, m.grade, um.\"user\" FROM movies.movie m, movies.keyword_movie km, movies.user_movie um WHERE km.movie = um.movie AND m.id = km.movie AND km.movie NOT IN (SELECT gm.movie FROM movies.genre_movie gm, movies.genre g WHERE g.title = 'Adult' AND g.id = gm.genre) AND km.keyword = ".$_GET['keyword']." AND (um.\"user\" = ".$user[0]['id'];
	  if ($others)
	  {
	    $others2 = array();
	    $own = 0;
  ...
  {
	  $SQL = "SELECT m.title, m.id, m.year, m.grade FROM movies.movie m, movies.keyword_movie km, movies.user_movie um WHERE km.movie = um.movie AND m.id = km.movie AND km.movie NOT IN (SELECT gm.movie FROM movies.genre_movie gm, movies.genre g WHERE g.title = 'Adult' AND g.id = gm.genre) AND km.keyword = ".$_GET['keyword']." ORDER BY m.title";
	}
	menu();
