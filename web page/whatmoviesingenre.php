...
if (isset($_GET['genre']))
{
    ...
    $SQL = "SELECT m.title, m.id, m.year, m.grade, um.\"user\" FROM movies.movie m, movies.genre_movie gm, movies.user_movie um WHERE m.id = gm.movie AND m.id NOT IN (SELECT gm.movie FROM movies.genre_movie gm, movies.genre g WHERE g.title = 'Adult' AND g.id = gm.genre) AND gm.movie = um.movie AND gm.genre = ".$_GET['genre']." AND (um.\"user\" = ".$user[0]['id'];
	  if ($others)
	  {
	    $others2 = array();
	    $own = 0;
